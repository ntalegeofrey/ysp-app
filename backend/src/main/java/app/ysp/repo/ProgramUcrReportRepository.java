...
public interface ProgramUcrReportRepository extends JpaRepository<ProgramUcrReport, Long> {
    ...
    // Count ALL individual unresolved issues (Critical + High + Medium) - counts each field separately across all reports
    @Query(value = "SELECT SUM(" +
           "  (CASE WHEN LOWER(r.security_radios_condition) LIKE '%critical%' OR LOWER(r.security_radios_condition) LIKE '%high%' OR LOWER(r.security_radios_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.security_flashlights_condition) LIKE '%critical%' OR LOWER(r.security_flashlights_condition) LIKE '%high%' OR LOWER(r.security_flashlights_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.security_metal_detector_condition) LIKE '%critical%' OR LOWER(r.security_metal_detector_condition) LIKE '%high%' OR LOWER(r.security_metal_detector_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.security_big_set_keys_condition) LIKE '%critical%' OR LOWER(r.security_big_set_keys_condition) LIKE '%high%' OR LOWER(r.security_big_set_keys_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.security_first_aid_kits_condition) LIKE '%critical%' OR LOWER(r.security_first_aid_kits_condition) LIKE '%high%' OR LOWER(r.security_first_aid_kits_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.security_desk_computer_condition) LIKE '%critical%' OR LOWER(r.security_desk_computer_condition) LIKE '%high%' OR LOWER(r.security_desk_computer_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.admin_meeting_rooms_locked_condition) LIKE '%critical%' OR LOWER(r.admin_meeting_rooms_locked_condition) LIKE '%high%' OR LOWER(r.admin_meeting_rooms_locked_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.admin_doors_secure_condition) LIKE '%critical%' OR LOWER(r.admin_doors_secure_condition) LIKE '%high%' OR LOWER(r.admin_doors_secure_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_back_door_condition) LIKE '%critical%' OR LOWER(r.infra_back_door_condition) LIKE '%high%' OR LOWER(r.infra_back_door_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_entrance_exit_doors_condition) LIKE '%critical%' OR LOWER(r.infra_entrance_exit_doors_condition) LIKE '%high%' OR LOWER(r.infra_entrance_exit_doors_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_smoke_detectors_condition) LIKE '%critical%' OR LOWER(r.infra_smoke_detectors_condition) LIKE '%high%' OR LOWER(r.infra_smoke_detectors_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_windows_secure_condition) LIKE '%critical%' OR LOWER(r.infra_windows_secure_condition) LIKE '%high%' OR LOWER(r.infra_windows_secure_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_laundry_area_condition) LIKE '%critical%' OR LOWER(r.infra_laundry_area_condition) LIKE '%high%' OR LOWER(r.infra_laundry_area_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_fire_extinguishers_condition) LIKE '%critical%' OR LOWER(r.infra_fire_extinguishers_condition) LIKE '%high%' OR LOWER(r.infra_fire_extinguishers_condition) LIKE '%medium%' THEN 1 ELSE 0 END) + " +
           "  (CASE WHEN LOWER(r.infra_fire_alarm_condition) LIKE '%critical%' OR LOWER(r.infra_fire_alarm_condition) LIKE '%high%' OR LOWER(r.infra_fire_alarm_condition) LIKE '%medium%' THEN 1 ELSE 0 END) " +
           ") FROM program_ucr_reports r " +
           "WHERE r.program_id = :programId AND (r.resolved = false OR r.resolved IS NULL)", nativeQuery = true)
